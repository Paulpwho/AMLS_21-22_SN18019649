...
tol = [x for x in np.logspace(start=-10, stop=1, num=10)]
max_iter = [int(x) for x in np.linspace(100, 1000, num = 90)]
...
kmeans_random = RandomizedSearchCV(estimator=kmeans,
...
kmeans_random.fit(xTrain, yTrain)
print(kmeans_random.best_params_)
y_pred_hyperparams = kmeans_random.predict(xTest)

# base model

base_model = KMeans(n_clusters=2, random_state=0)
